...
`visualizations/dashboard.py` generates the following PNGs into `output/`. Placeholders shown reference the eventual `docs/images/` copies for use in longer documentation; the live outputs are the files under `output/`. Every plot renders at 300 DPI, uses seaborn `whitegrid`, and applies `tight_layout`.
